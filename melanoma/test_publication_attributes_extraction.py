...
from src.domain.models import ChunkingConfiguration, ChunkWithEmbedding, EmbeddingConfiguration
...
async def test_publication_attributes(publication_file: str):
    ...
    if test_results:
        logger.info(f"✅ Found {len(test_results)} chunks for {publication_id} in vector store")
    else:
        logger.warning(f"⚠️  No chunks found for {publication_id}. Publication may not be loaded in vector store.")
        logger.info("   Note: You may need to run demo_publication_extraction.py first to load publications")
    ...
    for attr_type in test_attributes:
        ...
        try:
            ...
            extraction_results[attr_type] = {
                "value": extracted_attr.value if hasattr(extracted_attr, 'value') else str(extracted_attr),
                "confidence": extracted_attr.confidence if hasattr(extracted_attr, 'confidence') else 0.0,
                "source": extracted_attr.source if hasattr(extracted_attr, 'source') else "unknown",
            }
            
            logger.info(f"✅ Extracted: {extraction_results[attr_type]['value']}")
            logger.info(f"   Confidence: {extraction_results[attr_type]['confidence']:.2f}")
    ...
    for attr_type in test_attributes:
        ...
        if expected_value.lower() in str(actual_value).lower() or str(actual_value).lower() in expected_value.lower():
            logger.info(f"✅ {attr_type.value}: PASS (Expected: {expected_value}, Got: {actual_value})")
        else:
            logger.warning(f"⚠️  {attr_type.value}: MISMATCH (Expected: {expected_value}, Got: {actual_value})")
