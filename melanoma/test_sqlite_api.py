...
async def test_sqlite_api():
    ...
    async with httpx.AsyncClient(
        ...
        assert (
            service_type == "SQLiteTrialsService"
        ), f"Expected SQLiteTrialsService, got {service_type}"
        ...
        assert (
            data["total_abstracts"] == 978
        ), f"Expected 978, got {data['total_abstracts']}"
        ...
        assert (
            data["total_abstracts"] == 69
        ), f"Expected 69 publications, got {data['total_abstracts']}"

        # Test 4: Filtering by resource type (conference)
        print("\n4. Testing filtering by resource_type=conference...")
        response = await client.get(
            "/api/analytics/data?resource_type=conference&limit=10"
        )
        assert response.status_code == 200
        data = response.json()
        print(f"   ✅ Status: {response.status_code}")
        ...
        assert (
            data["total_abstracts"] == 909
        ), f"Expected 909 conference abstracts, got {data['total_abstracts']}"
        ...
        id1 = data1["abstracts"][0].get("abstract_id") or data1["abstracts"][0].get(
            "publication_id"
        )
        id2 = data2["abstracts"][0].get("abstract_id") or data2["abstracts"][0].get(
            "publication_id"
        )
        ...
        response = await client.get(
            "/api/analytics/chart-data?target_metric=MEDIAN_OS&limit=10"
        )
        assert response.status_code == 200
        ...
        if data["abstracts"]:
            ...
            assert (
                len(abstract["arm_results"]) > 0
            ), "Abstract should have at least one arm"
            print(
                f"   ✅ Abstract ID: {abstract.get('abstract_id') or abstract.get('publication_id')}"
            )
            print(f"   ✅ Arms: {len(abstract['arm_results'])}")
            print(
                f"   ✅ Attributes extracted: {abstract.get('total_attributes_extracted', 0)}"
            )
