        ...
        for chunk in chunks:
            ...
            if chunk_type == "methods":
                ...
                if pattern.lower() in chunk.content.lower():
                    print("  ✓ Pattern found in Methods chunk")
        ...
        try:
            ...
            for i, result in enumerate(direct_results[:3], 1):
                ...
                content = (
                    ...
                    else str(result.chunk)
                )
                has_pattern = pattern.lower() in content.lower()
                marker = "✓" if has_pattern else " "
                print(
                    f"    {marker} Result {i}: type={chunk_type}, len={len(content)}, has_pattern={has_pattern}"
                )
                if has_pattern:
            ...
            if methods_chunk:
                ...
                print(
                    f"  Methods chunk passes keyword filter: {'✓' if should_include else '❌'}"
                )
                if not should_include:
                    print(
                        f"    Methods chunk content preview: {methods_chunk.content[:200]}..."
                    )
            ...
            if context_texts:
                ...
                for i, text in enumerate(context_texts[:5], 1):
                    ...
                    is_results = any(
                        keyword in text.lower()[:100]
                        for keyword in ["results", "response", "efficacy", "outcome"]
                    )
                    ...
                    print(
                        f"    {marker} Chunk {i} [{chunk_type_label}]: {len(text)} chars"
                    )
                    if has_pattern:
                ...
                if priority_order and priority_order[0] == "methods":
                    print("    ✓ Methods chunk is first (correct prioritization)")
                elif methods_found:
                    methods_pos = next(
                        (i for i, ct in enumerate(priority_order) if ct == "methods"),
                        -1,
                    )
                    if methods_pos > 0:
                        print(
                            f"    ⚠️  Methods chunk at position {methods_pos + 1} (should be first)"
                        )
                else:
                    print("    ❌ Methods chunk not retrieved")
            else:
