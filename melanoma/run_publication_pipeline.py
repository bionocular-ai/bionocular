...
def _save_results(
    ...
    json_results: dict = {
        ...
        "total_attributes_extracted": sum(
            r.total_attributes_extracted for r in all_results
        )
        if all_results
        else 0,
        "average_confidence": sum(r.overall_confidence for r in all_results)
        / len(all_results)
        if all_results
        else 0,
        "total_processing_time_ms": sum(r.processing_time_ms for r in all_results)
        if all_results
        else 0,
        "publications": [],
    }
...
async def main():
    ...
    try:
        ...
        if TEST_MODE and TEST_FILE_OVERRIDE:
            publication_files = [
                f for f in publication_files if f.name == TEST_FILE_OVERRIDE
            ]
            if not publication_files:
                logger.error(
                    f"TEST_FILE_OVERRIDE '{TEST_FILE_OVERRIDE}' not found in {PUBLICATIONS_DIR}"
                )
                return
            ...
            logger.info(
                f"(TEST MODE: limiting to {MAX_PUBLICATIONS_TEST} publications)"
            )
        ...
        for pub_file in publication_files:
            ...
            all_publications_metadata.append(
                {"file": pub_file, "pub_id": pub_id, "content": content}
            )

        logger.info(
            f"Loaded content for {len(all_publications_metadata)} publication(s)"
        )
        ...
        if resume_path and resume_path.exists():
            ...
            logger.info(
                f"Resuming — skipping {len(already_done)} pub(s) already in {resume_path.name}"
            )
        ...
        processed_metadata: list = (
            []
        )  # parallel to all_results — only actually-processed pubs
        ...
        for idx, pub_meta in enumerate(all_publications_metadata):
            ...
            logger.info(f"\n{'=' * 60}")
            logger.info(
                f"PROCESSING PUBLICATION {idx + 1}/{len(all_publications_metadata)}: {pub_id}"
            )
            logger.info(f"{'=' * 60}")

            try:
                result = await extraction_service.extract(
                    content, pub_id, DocumentType.PUBLICATION
                )
            ...
            _save_results(
                all_results, processed_metadata, output_file, TEST_MODE, allowed_fields
            )
            cost_calculator.save_detailed_report(str(cost_report_file))
            remaining = len(all_publications_metadata) - len(already_done)
            logger.info(
                f"  [incremental] Saved {len(all_results)}/{remaining} pub(s) → {output_file.name}"
            )
