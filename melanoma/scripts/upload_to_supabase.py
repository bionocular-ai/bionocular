...
def upload_clinical_trials():
    ...
    for i in range(0, total, batch_size):
        ...
        for item in batch:
            ...
            try:
                raw_json = (
                    json.loads(raw_str) if isinstance(raw_str, str) else dict(raw_str)
                )
            except Exception:
                raw_json = {}

            processed_sql_record = extract_processed_trial(
                raw_json, nct_id, cancer_types_map
            )
        ...
        try:
            ...
            print(
                f"  ✓ Inserted batch {i//batch_size + 1} / {(total + batch_size - 1) // batch_size} into clinical_trials"
            )
        except Exception as e:
...
def upload_trial_landscape():
    ...
    for record in data:
        ...
        if isinstance(raw_ct, str):
            ...
            record["cancer_type"] = [
                normalize_cancer_type(p)[0] for p in parts if normalize_cancer_type(p)
            ]
        ...
        with open(path) as f:
            try:
                ...
                for trial in trials_list:
                    ...
                    raw_id = (
                        trial.get("trial_id")
                        or trial.get("abstract_id")
                        or trial.get("pub_id")
                        or trial.get("publication_id")
                        or trial.get("id", "unknown")
                    )
                    nct_id = trial.get("nct_id") or (
                        raw_id if str(raw_id).startswith("NCT") else None
                    )

                    # Publication & Arm specific Logic
                    arm_results = trial.get("arm_results", {})
                    first_arm_key = (
                        next(iter(arm_results.keys())) if arm_results else None
                    )
                    first_arm_attrs = (
                        arm_results[first_arm_key].get("attributes", {})
                        if first_arm_key
                        else {}
                    )
                    ...
                    for arm_key, arm_data in arm_results.items():
                        ...
                        pk = f"{source_type}_{raw_id}_{arm_id}".replace(
                            "/", "_"
                        ).replace(" ", "_")
                        ...
                        for k, v in attrs.items():
                            clean_k = (
                                k.lower().replace("attributetype.", "").replace("_", "")
                            )
                ...
                if mapped_arms:
                    ...
                    print(
                        f"  ✓ Inserted {len(mapped_arms)} deeply flattened arms from {file}"
                    )
