...
class JSONTrialsService:
    ...
    def _load_json_files(self) -> list[dict[str, Any]]:
        ...
        current_timestamps = {
            str(p): p.stat().st_mtime for p in self.json_file_paths if p.exists()
        }
        ...
        if self._cache is None or self._cache_timestamps != current_timestamps:
            ...
            for json_file_path in self.json_file_paths:
                ...
                try:
                    ...
                    if filtered_count > 0:
                        logger.info(
                            f"Filtered out {filtered_count} item(s) with no treatment arms from {json_file_path.name}"
                        )
            ...
            logger.info(
                f"Total abstracts loaded: {total_loaded} from {len(self.json_file_paths)} file(s)"
            )
    ...
    def _extract_trial_from_abstract(self, abstract: dict[str, Any]) -> dict[str, Any]:
        ...
        abstract_id = (
            abstract.get("publication_id", "")
            if is_publication
            else abstract.get("abstract_id", "")
        )
        if not abstract_id:
            abstract_id = self._extract_attribute_value(
                attributes, "AttributeType.ABSTRACT_NUMBER"
            )
        ...
        result = {
            ...
            "type": "publication"
            if is_publication
            else "abstract",  # Track if it's a publication or abstract
        }
    ...
    def get_full_abstract_by_id(self, abstract_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.error(
                f"Error finding full abstract by abstract_id: {e}", exc_info=True
            )
            return None

    def _normalize_cancer_types_in_abstract(
                ...
                if key in attributes:
                    ...
                    if isinstance(attr_data, dict):
                        ...
                        if original_value:
                            ...
                            normalized_types = normalize_cancer_type_with_splitting(
                                original_value
                            )
    ...
    def get_trials_by_nct_id(
        ...
        try:
            ...
            for abstract in abstracts:
                ...
                abstract_nct = self._extract_attribute_value(
                    attributes, "AttributeType.NCT_NUMBER"
                )
                ...
                if (
                    abstract_nct
                    and abstract_nct.upper().strip() == nct_id.upper().strip()
                ):
