    ...
    async def extract_text(self, file_content: bytes) -> str:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Successfully extracted text using Marker (length: {len(final_text)})")
                return final_text
    ...
    async def extract_metadata(self, file_content: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Successfully extracted metadata using Marker")
                return metadata
            ...
            for i, page_data in enumerate(rendered.children, 1):
                ...
                if hasattr(page_data, 'children') and page_data.children:
                    for block in page_data.children:
                        if hasattr(block, 'block_type') and block.block_type == "PageHeader":
                            if hasattr(block, 'html') and block.html:
                                import re
                                clean_text = re.sub(r'<[^>]+>', '', block.html).strip()
                                if clean_text and clean_text != "Text that appears at the top of a page, like a page title.":
                ...
                if not page_headers and hasattr(page_data, 'html') and page_data.html:
                    ...
                    for pattern in header_patterns:
                        ...
                        if clean_pattern and len(clean_pattern) > 5 and clean_pattern.isupper():
