...
class ESMOPostprocessor(PostprocessorInterface):
    ...
    def _normalize_latex_symbols(self, text: str) -> str:
        ...
        cleaned = re.sub(
            r"\$(-?\d+\.?\d*)\s*\\pm\s*(\d+\.?\d*)\s*\$", r"\1 ± \2", cleaned
    ...
    def clean_section_text(self, text: str) -> str:
        ...
        cleaned = re.sub(
            r"\s*\*\*S\d+\*\*\s*Volume \d+ ■ Issue S\d+ ■ \d{4}\s*", " ", cleaned
        )
        cleaned = re.sub(r"\s*\*\*S\d+\*\*\s*", " ", cleaned)  # Just the S number part
        cleaned = re.sub(
            r"\s*Volume \d+ ■ Issue S\d+ ■ \d{4}\s*", " ", cleaned
        )  # Just the volume part
    ...
    async def parse_abstract(self, abstract_text: str) -> ParsedAbstract:
        ...
        if legal_match:
            ...
            legal_text = re.sub(
                r"\s*\*\*S\d+\*\*\s*Volume \d+ ■ Issue S\d+ ■ \d{4}\s*", " ", legal_text
            )
            legal_text = re.sub(r"\s*\*\*S\d+\*\*\s*", " ", legal_text)
            legal_text = re.sub(
                r"\s*Volume \d+ ■ Issue S\d+ ■ \d{4}\s*", " ", legal_text
            )
        ...
        if funding_match:
            ...
            funding_text = re.sub(
                r"\*\*Disclosure:.*$", "", funding_text, flags=re.DOTALL | re.IGNORECASE
            )
            funding_text = re.sub(
                r"Disclosure:.*$", "", funding_text, flags=re.DOTALL | re.IGNORECASE
            )
            ...
            funding_text = re.sub(
                r"\s*\*\*S\d+\*\*\s*Volume \d+ ■ Issue S\d+ ■ \d{4}\s*",
                " ",
                funding_text,
            )
            funding_text = re.sub(r"\s*\*\*S\d+\*\*\s*", " ", funding_text)
            funding_text = re.sub(
                r"\s*Volume \d+ ■ Issue S\d+ ■ \d{4}\s*", " ", funding_text
            )
        ...
        if ack_match:
            ...
            ack_content = re.sub(
                r"^Editorial acknowledgement:\s*\*?\*?\s*",
                "",
                ack_content,
                flags=re.IGNORECASE,
            )
            ...
            ack_content = re.sub(
                r"\*\*Legal entity responsible for the study:\*\*.*?Funding:.*?(?=\Z|$)",
                "",
                ack_content,
                flags=re.DOTALL | re.IGNORECASE,
            )
            ...
            ack_content = re.sub(
                r"Funding:.*?$", "", ack_content, flags=re.DOTALL | re.IGNORECASE
            )
            ...
            ack_content = re.sub(
                r"\s*Volume \d+ ■ Issue S\d+ ■ \d{4}\s*", " ", ack_content
            )
        ...
        if markdown_match:
            ...
            url_match = re.search(
                r"\(https://doi\.org/10\.1016/[ij]\.annonc\.\d{4}\.\d{2}\.\d{4}\)",
                markdown_match.group(0),
            )
            if url_match:
                ...
                bracket_match = re.search(
                    r"\[(10\.1016/[ij]\.annonc\.\d{4}\.\d{2}\.\d{4})\]",
                    markdown_match.group(0),
                )
                if bracket_match:
        ...
        for original_line in lines:
            ...
            if (
                not line_stripped
                or "melanoma and other skin tumours" in line_stripped.lower()
            ):
                continue
            ...
            if not header_ended:
                ...
                for m in section_markers:
                    ...
                    if pos != -1:
                        ...
                        stars_before = lower_line[
                            : pos + lower_line.count("**", 0, pos) * 2
                        ].count("**")
                ...
                if line_stripped.startswith("# ") and re.search(
                    r"\d{3,5}[A-Z]*", line_stripped
                ):
                    break
        ...
        for i, line in enumerate(lines):
            ...
            if (
                re.match(r"^\*\*S\d+\*\*\s*Volume", cleaned_line)
                or re.match(r"^Volume \d+ ■ Issue S\d+ ■ \d{4}", cleaned_line)
                or cleaned_line.startswith("<sup>")
                or re.match(r"^[A-Z]\..*", line)
                or len(cleaned_line) < 10
            ):
                continue
            ...
            if len(cleaned_line) > 20:
                if not title_found:
                    ...
                    if parsed_data["title"].endswith("KEYMAKER-"):
                        parsed_data["title"] = self.clean_text(
                            f"{parsed_data['title']} Sequential targeted and immunotherapies in stage IV"
                        )
                    # Check if title ends with "-" and next line might be continuation
                        ...
                        if (
                            ...
                            and not next_line.startswith("<sup>")
                            and not re.match(r"^\*\*S\d+\*\*\s*Volume", next_line)
                        ):
                            parsed_data["title"] = self.clean_text(
                                f"{parsed_data['title']} {next_line}"
                            )
                    break
            elif (
                title_found
                and len(cleaned_line) > 10
        ...
        if parsed_data["title"] != "Title not found":
            parsed_data["title"] = re.sub(
                r"\s*\*\*S\d+\*\*\s*Volume \d+ ■ Issue S\d+ ■ \d{4}\s*",
                " ",
                parsed_data["title"],
            )
            parsed_data["title"] = re.sub(
                r"\s*\*\*S\d+\*\*\s*", " ", parsed_data["title"]
            )
            parsed_data["title"] = re.sub(
                r"\s*Volume \d+ ■ Issue S\d+ ■ \d{4}\s*", " ", parsed_data["title"]
            )
        ...
        if table_lines:
            ...
            parsed_data["additional_content"] = self.clean_table_content(
                table_content
