...
async def test_year(year):
    ...
    try:
        result = await service.process_file(str(input_file), str(output_file), config)
...
async def main():
    ...
    for year, result in results:
        print(
            f"ESMO {year}: {result.abstracts_processed} abstracts, {result.abstracts_with_warnings} warnings"
        )
    ...
    for file_path, abstract_id, expected_section in test_cases:
        ...
        abstract_match = re.search(
            rf"### Abstract ID: {re.escape(abstract_id)}\n(.*?)(?=\n### Abstract ID:|\Z)",
            content,
            re.DOTALL,
        )
        ...
        if abstract_match:
            ...
            if expected_section in sections:
                print(
                    f"✅ {file_path} - Abstract {abstract_id}: {expected_section} section is present"
                )
            else:
                print(
                    f"❌ {file_path} - Abstract {abstract_id}: {expected_section} section is MISSING"
                )
