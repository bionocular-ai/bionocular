...
os.environ["TRIALS_DB_PATH"] = str(Path(__file__).parent / "data" / "trials_db" / "trials.db")
...
async def test_frontend_api_calls():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        response = await client.get("/api/analytics/data?resource_type=publication&limit=2000")
        assert response.status_code == 200
        data = response.json()
        ...
        for abstract in data['abstracts'][:5]:  # Check first 5
            assert abstract.get('publication_id') is not None, "Should have publication_id"
            assert abstract.get('abstract_id') is None or abstract.get('file') is None, "Should not have abstract_id or file"
        
        # Test 3: Filter by resource_type (conference)
        print("\n3. Testing resource_type=conference filter...")
        response = await client.get("/api/analytics/data?resource_type=conference&limit=2000")
        assert response.status_code == 200
        data = response.json()
        print(f"   ✅ Conference abstracts: {data['total_abstracts']}")
        # Verify all returned items are conference abstracts
        for abstract in data['abstracts'][:5]:  # Check first 5
            assert abstract.get('abstract_id') is not None or abstract.get('file') is not None, "Should have abstract_id or file"
        
        # Test 4: Filter by therapy_type
        print("\n4. Testing therapy_type filter...")
        response = await client.get("/api/analytics/data?therapy_type=Immunotherapy&limit=2000")
        assert response.status_code == 200
        data = response.json()
        ...
        response = await client.get("/api/analytics/data?funding_type=industry&limit=2000")
        assert response.status_code == 200
        data = response.json()
        ...
        response = await client.get("/api/analytics/data?resource_type=publication&therapy_type=Immunotherapy&funding_type=industry&limit=2000")
        assert response.status_code == 200
        data = response.json()
        ...
        required_fields = ['abstracts', 'total_abstracts', 'total_arms', 'total_attributes_extracted', 'average_confidence']
        ...
        if data['abstracts']:
            ...
            assert isinstance(abstract['arm_results'], dict), "arm_results should be a dict"
        ...
        response_all = await client.get("/api/analytics/data?resource_type=publication&limit=2000")
        data_all = response_all.json()
        total_publications = data_all['total_abstracts']
        
        # Get first page
        response_page1 = await client.get("/api/analytics/data?resource_type=publication&limit=10")
        data_page1 = response_page1.json()
        
        # Verify all items in page1 are publications
        for abstract in data_page1['abstracts']:
            assert abstract.get('publication_id') is not None, "All items should be publications"
        
        print(f"   ✅ Filter applied correctly: {len(data_page1['abstracts'])} publications on page 1")
