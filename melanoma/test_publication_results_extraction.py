...
async def test_results_extraction_and_arm_separation(publication_file: str):
    ...
    logger.info(f"   This is {len(results_section)/len(pub_content)*100:.1f}% of full publication")
    ...
    arm_indicators = [
        "arm", "group", "treatment", "received", "randomly assigned",
        "ipilimumab", "nivolumab", "pembrolizumab", "dose", "mg/kg"
    ]
    
    results_lower = results_section.lower()
    found_indicators = [ind for ind in arm_indicators if ind in results_lower]
    
    logger.info(f"✅ Found {len(found_indicators)}/{len(arm_indicators)} arm indicators:")
    for ind in found_indicators[:5]:
        logger.info(f"   - '{ind}'")
    
    if len(found_indicators) < 3:
        logger.warning("⚠️  Few arm indicators found - Results section might not contain arm information")
    ...
    try:
        ...
        logger.info(f"✅ Arm separation completed!")
