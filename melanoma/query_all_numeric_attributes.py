...
from src.app.langchain_factory_service import LangChainServiceFactory, ServiceConfiguration
...
ATTRIBUTE_KEYWORDS = {
    # PFS Family
    AttributeType.MEDIAN_PFS: ["pfs", "progression-free survival", "progression free survival"],
    AttributeType.MEDIAN_FOLLOWUP_PFS: [
        ["pfs", "progression-free", "progression free"],  # Group 1: Must have PFS
        ["follow-up", "followup", "follow up", "median follow"]  # Group 2: Must have follow-up (handles "3.05-yr median follow-up")
    ],
    ...
    AttributeType.PFS_RATE_12M: [
        ["pfs", "progression-free", "progression free"],
        ["12 month", "12 months", "12 mo", "12mo", "12m", "1 year", "1 years", "1 yr", "1yr", "1 y", "1y", "12 mth", "12 mths", "one year", "twelve month", "twelve months"]
    ],
    ...
    AttributeType.PFS_RATE_24M: [
        ["pfs", "progression-free", "progression free"],
        ["24 month", "24 months", "24 mo", "24mo", "24m", "2 year", "2 years", "2 yr", "2yr", "2 y", "2y", "24 mth", "24 mths", "two year", "two years"]
    ],
    AttributeType.PFS_RATE_36M: [
        ["pfs", "progression-free", "progression free"],
        ["36 month", "36 months", "36 mo", "36mo", "36m", "3 year", "3 years", "3 yr", "3yr", "3 y", "3y", "36 mth", "36 mths", "three year", "three years"]
    ],
    AttributeType.PFS_RATE_48M: [
        ["pfs", "progression-free", "progression free"],
        ["48 month", "48 months", "48 mo", "48mo", "48m", "4 year", "4 years", "4 yr", "4yr", "4 y", "4y", "48 mth", "48 mths", "four year", "four years"]
    ],
    ...
    # Response Rates
    AttributeType.OBJECTIVE_RESPONSE_RATE: ["orr", "objective response rate", "response rate", "rr"],
    AttributeType.COMPLETE_RESPONSE: ["cr", "complete response"],
    AttributeType.PATHOLOGICAL_COMPLETE_RESPONSE: ["pcr", "pathological complete response", "pathologic complete response"],
    ...
    AttributeType.RFS: ["rfs", "recurrence-free survival", "recurrence free survival", "relapse-free survival"],
    ...
    AttributeType.HR_RFS: [
        ["rfs", "recurrence-free", "recurrence free", "relapse-free", "relapse free"],  # Group 1: Must have RFS
        ["hr", "hazard ratio"]                                                           # Group 2: Must have HR
    ],
    ...
    AttributeType.NUMBER_OF_PATIENTS: ["patient", "pts", "enrolled", "randomized", "n="],
...
class ComprehensiveRetrievalTester:
    ...
    async def index_abstracts(self, file_path: str):
        ...
        for i, abstract_content in enumerate(abstracts, 1):
            ...
            print(f"  [{i}/{self.num_abstracts}] Indexing abstract {abstract_id}...", end='\r')
    ...
    async def test_attribute_retrieval(
        ...
        # Get configuration
        required_chunk_types = RAGOptimizationConfig.get_required_chunk_types(attribute_type)
        queries = self.query_templates.get(attribute_type, [f"{attribute_type.value} data"])
        ...
        try:
            ...
            if results:
                ...
                if keywords:
                    filtered_results, rejected_results = filter_results_by_keywords(results, keywords)
                ...
                for result in results:
                    ...
                    chunks_data.append({
                        "content": result.chunk.content[:1000],  # Limit to 1000 chars
                        "chunk_type": result.chunk.chunk_type.value,
                        "similarity_score": result.similarity_score,
                        "passed_filter": is_filtered,
                        "sequence_number": result.chunk.sequence_number,
                        "metadata": {
                            "is_subchunk": result.chunk.metadata.get("is_subchunk", False),
                            "subchunk_index": result.chunk.metadata.get("subchunk_index"),
                            "total_subchunks": result.chunk.metadata.get("total_subchunks"),
                        }
                    })
    ...
    async def run_comprehensive_test(self):
        ...
        print(f"{'='*80}")
        print(f"Testing {len(RAGOptimizationConfig.NUMERIC_ATTRIBUTES)} numeric attributes")
        print(f"Across {len(self.indexed_abstract_ids)} abstracts")
        print(f"Total queries: {len(RAGOptimizationConfig.NUMERIC_ATTRIBUTES) * len(self.indexed_abstract_ids)}")
        print(f"{'='*80}\n")
        
        all_results = []
        total_queries = len(RAGOptimizationConfig.NUMERIC_ATTRIBUTES) * len(self.indexed_abstract_ids)
        current_query = 0
        
        for attribute_type in sorted(RAGOptimizationConfig.NUMERIC_ATTRIBUTES, key=lambda x: x.value):
            for abstract_id in self.indexed_abstract_ids:
                current_query += 1
                
                print(f"[{current_query}/{total_queries}] Testing {attribute_type.value:40s} in {abstract_id}...", end='\r')
                
                result = await self.test_attribute_retrieval(attribute_type, abstract_id, max_chunks=5)
    ...
    def generate_report(self, results: List[Dict[str, Any]], output_file: str = None):
        ...
        print(f"\n{'='*80}")
        print(f"📊 RETRIEVAL ANALYSIS REPORT")
        print(f"{'='*80}\n")
        
        # Overall statistics
        total_queries = len(results)
        queries_with_results = sum(1 for r in results if r.get("unfiltered_count", 0) > 0)
        queries_with_keywords = sum(1 for r in results if r.get("has_keywords", False))
        queries_filtered_to_zero = sum(1 for r in results if r.get("filtered_count", 0) == 0 and r.get("unfiltered_count", 0) > 0 and r.get("has_keywords", False))
        
        total_unfiltered = sum(r.get("unfiltered_count", 0) for r in results)
        total_filtered = sum(r.get("filtered_count", 0) for r in results if r.get("has_keywords", False))
        ...
        print(f"   Queries with Keyword Filter: {queries_with_keywords} ({queries_with_keywords/total_queries*100:.1f}%)")
        print(f"   Queries Filtered to Zero:   {queries_filtered_to_zero} ({queries_filtered_to_zero/queries_with_keywords*100:.1f}% of filtered)")
        print(f"\n   Total Chunks Retrieved:     {total_unfiltered}")
        print(f"   Chunks Passed Filter:       {total_filtered}")
        print(f"   Chunks Rejected:            {total_rejected} ({total_rejected/total_unfiltered*100:.1f}% if total > 0)")
        
        # Attribute family statistics
        print(f"\n{'─'*80}")
        print(f"📊 By Attribute Family:")
        print(f"{'─'*80}\n")
        ...
        for family, keywords_list in families.items():
            ...
            if family_results:
                ...
                fam_with_results = sum(1 for r in family_results if r.get("unfiltered_count", 0) > 0)
                fam_filtered_zero = sum(1 for r in family_results if r.get("filtered_count", 0) == 0 and r.get("unfiltered_count", 0) > 0 and r.get("has_keywords", False))
                
                print(f"{family:20s}: {fam_with_results:3d}/{fam_total:3d} found data ({fam_with_results/fam_total*100:5.1f}%), "
        ...
        for abstract_id in self.indexed_abstract_ids:
            abstract_results = [r for r in results if r.get("abstract_id") == abstract_id]
            
            abs_with_results = sum(1 for r in abstract_results if r.get("unfiltered_count", 0) > 0)
            abs_attr_present = sum(1 for r in abstract_results if r.get("attribute_present", False))
            
            print(f"Abstract {abstract_id}: {abs_with_results}/{len(abstract_results)} queries returned chunks, "
                  f"{abs_attr_present} attributes confirmed present")
        
        # Most problematic attributes (high rejection rate)
        print(f"\n{'─'*80}")
        print(f"⚠️  Most Problematic Attributes (High False Positive Rate):")
        print(f"{'─'*80}\n")
        ...
        for i, (attr, rate, stats) in enumerate(rejection_rates[:10], 1):
            print(f"{i:2d}. {attr:40s}: {rate*100:5.1f}% rejected ({stats['rejected']}/{stats['unfiltered']} chunks)")
        
        # Attributes with no keyword filtering
        print(f"\n{'─'*80}")
        print(f"⚠️  Attributes WITHOUT Keyword Filtering:")
        print(f"{'─'*80}\n")
        
        no_keywords = sorted(set(r.get("attribute") for r in results if not r.get("has_keywords", False)))
        ...
        if no_keywords:
            ...
            print(f"   ✅ All attributes have keyword filtering!")
