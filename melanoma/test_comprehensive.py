...
async def test_response_times():
    ...
    async with httpx.AsyncClient(
        ...
        tests = [
            ...
            (
                "Filtered (publications, limit=100)",
                "/api/analytics/data?resource_type=publication&limit=100",
            ),
            (
                "Filtered (conference, limit=100)",
                "/api/analytics/data?resource_type=conference&limit=100",
            ),
            (
                "Chart data (MEDIAN_OS)",
                "/api/analytics/chart-data?target_metric=MEDIAN_OS",
            ),
            (
                "Filtered + chart data",
                "/api/analytics/chart-data?target_metric=MEDIAN_OS&resource_type=publication",
            ),
        ]
        ...
        for name, endpoint in tests:
            ...
            print(
                f"  {name:40s} {avg_time:6.1f}ms (min: {min(times):.1f}ms, max: {max(times):.1f}ms)"
            )
...
async def test_additional_filters():
    ...
    async with httpx.AsyncClient(
        ...
        response = await client.get(
            "/api/analytics/data?therapy_type=Immunotherapy&limit=10"
        )
        ...
        response = await client.get(
            "/api/analytics/data?funding_type=industry&limit=10"
        )
        assert response.status_code == 200
        data = response.json()
        ...
        response = await client.get(
            "/api/analytics/data?funding_type=non-industry&limit=10"
        )
        assert response.status_code == 200
        data = response.json()
        ...
        response = await client.get(
            "/api/analytics/data?resource_type=publication&therapy_type=Immunotherapy&funding_type=industry&limit=10"
        )
        assert response.status_code == 200
        data = response.json()
        print(
            f"   ✅ Combined (publication + immunotherapy + industry): {data['total_abstracts']} abstracts"
        )
        ...
        print(
            f"   ✅ Chart data with filter: {len(chart_data.get('treatmentGroups', []))} treatment groups"
        )
        print(
            f"   ✅ Summary abstracts: {chart_data.get('summary', {}).get('totalAbstracts', 0)}"
        )
        ...
        return {
            ...
            "funding_type": {
                ...
                "non_industry": non_industry_count,
            },
            "has_metric": True,
            "combined": True,
...
async def test_edge_cases():
    ...
    async with httpx.AsyncClient(
        ...
        response = await client.get(
            "/api/analytics/data?resource_type=invalid&limit=10"
        )
        assert (
            response.status_code == 200
        )  # Should not error, just return empty/filtered results
        data = response.json()
        print(
            f"   ✅ Invalid resource_type handled gracefully: {data['total_abstracts']} results"
        )

        # Test empty results
        print("\n2. Testing filter that returns no results...")
        response = await client.get(
            "/api/analytics/data?cancer_type=NonexistentCancer&limit=10"
        )
        assert response.status_code == 200
        data = response.json()
        print(
            f"   ✅ Empty results handled: {data['total_abstracts']} results, {len(data['abstracts'])} returned"
        )
        ...
        data = response.json()
        print(
            f"   ✅ Large limit handled: {len(data['abstracts'])} returned (total: {data['total_abstracts']})"
        )
        ...
        response = await client.get(
            "/api/analytics/data?cancer_type=Cutaneous%20Melanoma&limit=10"
        )
        assert response.status_code == 200
        data = response.json()
        ...
        data = response.json()
        print(
            f"   ✅ Missing parameters handled: {data['total_abstracts']} total, {len(data['abstracts'])} returned"
        )
...
async def run_all_tests():
    ...
    print("=" * 60)
    print(
        f"✅ Memory: {results['memory']['service_type']} - {results['memory']['memory_mb']:.2f} MB"
    )
    print(f"✅ Response Times: {len(results['response_times'])} tests completed")
    print("✅ Filters: All filter types tested")
    print("✅ Edge Cases: All edge cases handled")
