"""Comprehensive test for all clinical trial attributes."""
...
def test_all_attributes(nct_number: str = "NCT02362594"):
    ...
    """
    logger.info("=" * 100)
    logger.info(f"COMPREHENSIVE ATTRIBUTE TEST - {nct_number}")
    logger.info("=" * 100)
    ...
    logger.info("\n" + "-" * 100)
    logger.info("RAW TRIAL DATA (Additional Information)")
    logger.info("-" * 100)
    logger.info(f"Trial Name: {trial_data.trial_name}")
    logger.info(f"Clinical Trial Phase: {trial_data.clinical_trial_phase}")
    logger.info(f"Primary Endpoint: {trial_data.primary_endpoint}")
    logger.info(f"Secondary Endpoint: {trial_data.secondary_endpoint}")
    logger.info(f"Study Start Date: {trial_data.study_start_date}")
    logger.info(f"Study Completion Date: {trial_data.study_completion_date}")
    logger.info(f"First Results: {trial_data.first_results}")
    logger.info(f"Number of Patients: {trial_data.number_of_patients}")
    logger.info(f"Sponsors: {trial_data.sponsors}")
    logger.info(f"Cancer Type: {trial_data.cancer_type}")
    ...
    for i, arm in enumerate(trial_data.treatment_arms):
        ...
        logger.info(f"{'=' * 100}")
        logger.info(f"  Arm Type: {arm.arm_type}")
    ...
    total_attributes = len([a for a in attributes_to_test.values() if a])
