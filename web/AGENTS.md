...
This app is on Next.js 16 (App Router). The version-matched docs ship with the package at `node_modules/next/dist/docs/`; consult them for API details instead of assuming older Next.js behavior. `agentRules: false` in `next.config.ts` keeps `next dev` from writing its own rules block into this file; instructions here are repo-authored only.
